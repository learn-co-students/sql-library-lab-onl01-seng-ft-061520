...
def select_series_title_with_most_human_characters
  ...
  character_books.book_id = books.id JOIN characters ON character_books.character_id = characters.id WHERE characters.species = 'human' GROUP BY series.title ORDER BY COUNT(*) DESC LIMIT 1"
end
#so then we can reference that variable in our last line, and use it, like so:
#so then we can reference that variable in our last line, and use it, like so:
#and then it will be in the correct alphabetical order
